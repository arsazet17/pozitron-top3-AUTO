...
if start < 0 or end < 0: raise RuntimeError('strip block not found')
s = s[:start] + "function stripM4FromFinal(){ /* canonical Yulia core owns M1/M2/M3; M4/M6 stay separate. */ }" + s[end:]

boot_start = s.find('async function boot(){')
sync_start = s.find('\nasync function sync(manual=false){', boot_start)
if boot_start < 0 or sync_start < 0: raise RuntimeError('boot block not found')
...
if sync_start < 0 or sched_start < 0: raise RuntimeError('sync block not found')
...
print('OK: Analyzer v0.6.5 uses canonical Yulia M1/M2/M3 and shared M4/M5/M6 modules.')
