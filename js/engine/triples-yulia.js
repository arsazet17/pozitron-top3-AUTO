/* Canonical TOP-3 M1/M2/M3 engine.
 * This module intentionally follows Yulia TOP-3 triple-methods logic.
 * AUTO imports this single implementation through triples-chat.js so the
...
const SEED_STATE = {
  ...
  diag: {blocks:[],raw:[],packages:[],dependency:[],mirror:"002",mirrorPass:true,mirrorHits:null,m2Events:[],seriesEvents:[],serialEvents:[]}
};
...
function chronological(records=[]){
  return records
    .map(r=>({id:Number(r.draw??r.id),date:r.date,time:r.time,code:String(r.combo??((r.A!=null&&r.B!=null&&r.C!=null)?`${r.A}${r.B}${r.C}`:""))}))
    .filter(r=>Number.isInteger(r.id)&&/^\d{3}$/.test(r.code))
    .sort((a,b)=>a.id-b.id);
}

/* Yulia's history gate is a pure existence test. AUTO has the same complete
 * packed pre-history plus official dated records. Count only the packed
 * pre-dated part here and add earlierReal below: this is the same set without
 * double-counting the dated tail.
 */
...
function fmtSerial(a){return a&&a.length?a.map(x=>`${x.triple} · rem${x.rem}`).join(" / "):"—"}

/* Exact Yulia serial rule: activate once when the same addition birth occurs
 * in exactly two consecutive draws. Third and later consecutive births do not
 * restart/extend the five-draw window.
 */
export function advanceSerialLeaders(active=[],previousStreaks={},additionBirths=[],currentId=0){
  ...
  const additionStreaks={};
  for(const t of births){
    const prev=previousStreaks?.[t];
    const consecutive=prev&&Number(prev.lastId)===Number(currentId)-1;
    additionStreaks[t]={len:consecutive?Number(prev.len||1)+1:1,lastId:Number(currentId)};
  }
  const serialLeaders=(active||[]).map(x=>({...x,rem:Number(x.rem||0)-1})).filter(x=>x.rem>0);
  const events=[];
  for(const [t,streak] of Object.entries(additionStreaks)){
    if(streak.len===2&&!serialLeaders.some(x=>x.triple===t)){
      serialLeaders.push({triple:t,rem:SERIAL_LEADER_TTL,startedAt:Number(currentId),rule:"addition-streak"});
      events.push(`${t}: сложением 2 тиража подряд → ЛИДЕР на ${SERIAL_LEADER_TTL} следующих тиражей`);
    }
  }
  serialLeaders.sort((a,b)=>tripleSort(a.triple,b.triple));
  return {active:serialLeaders,streaks:additionStreaks,events};
}
...
function processForward(state,current,allReal,counts){
  ...
  Object.assign(state,{
    ...
    additionStreaks:serial.streaks,serialLeaders:serial.active,frozen,diag
  });
  state.archive.push({
