...
export function renderHome(ctx){
 const issue=ctx.issue,last=ctx.records.at(-1),m=issue.methods||{},sig=m.main?.signals||{},fullCount=ctx.fullArchive?.total||ctx.records.length;
 ...
 ${card("СТАТУС СИСТЕМЫ",`<div class="list"><div class="list-row"><span>Ядро</span><b>CHAT MASTER</b></div><div class="list-row"><span>Версия</span><b>${ctx.version.version}</b></div><div class="list-row"><span>Фактов · полный архив</span><b>${fullCount}</b></div><div class="list-row"><span>С датой / временем</span><b>${ctx.records.length}</b></div><div class="list-row"><span>Аудитов MASTER</span><b>${ctx.state?.masterAudit?.length||0}</b></div><div class="list-row"><span>Баз зеркала</span><b>${ctx.mirror?.bases?.length||0}</b></div></div>`)}
 </div>
